...
// GenerateSchemaToFile generates a YAML schema to a specific file path.
func (g *SchemaGenerator) GenerateSchemaToFile(componentKind, componentName, configTypeName, configPkgPath, outputPath string) error {
	structInfo, err := g.analyzer.AnalyzeConfig(configTypeName, configPkgPath)
	if err != nil {
		return fmt.Errorf("failed to analyze config: %w", err)
	}
...
// StructToSchema converts a StructInfo to a JSON Schema (exported for testing).
func (g *SchemaGenerator) StructToSchema(info *StructInfo, componentKind, componentName string) *Schema {
	return g.structToSchema(info, componentKind, componentName)
}
...
// SetSchemaType sets the JSON Schema type based on Go type (exported for testing).
func (g *SchemaGenerator) SetSchemaType(schema *Schema, goType string) {
	g.setSchemaType(schema, goType)
}
